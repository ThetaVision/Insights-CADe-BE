...
class FCBFormer(nn.Module):
    def __init__(self, opt):
        ...
        self.TB = TB(version=opt.backbone)

        self.FCB = FCB(in_resolution=opt.imagesize)
        self.PH = nn.Sequential(RB(64 + 32, 64), RB(64, 64), nn.Conv2d(64, 1, kernel_size=1))
        self.up_tosize = nn.Upsample(size=opt.imagesize)
        ...
        if opt.imagesize == 256:
            ...
            self.avgpool = nn.AvgPool2d(kernel_size=(88, 88))
        self.fc = nn.Linear(96 * 4 * 4, opt.num_classes)
    ...
    def forward_features_list(self, x):
        ...
        # Intermediate features TB
        feat_tb = self.TB.get_pyramid(x)

        # Intermediate features FCB
        feat_fcb = self.FCB.get_pyramid(x)

        # Concatenate features
        for j in range(len(feat_tb)):
            features.append(torch.cat([feat_tb[j], feat_fcb[j]], dim=1))
...
class TB(nn.Module):
    def __init__(self, version):
        ...
        if version == 'FCBFormer-B0':
        ...
        elif version == 'FCBFormer-B2-Li':
            # Import PVT-V2-B2-Linear
            backbone = PyramidVisionTransformerV2(
                patch_size=4,
                embed_dims=[64, 128, 320, 512],
                num_heads=[1, 2, 5, 8],
            ...
            raise Exception('Unexpected Version of FCBFormer: {}'.format(version))
...
"""""" """""" """""" """""" """""" """""" """"""
"""" DEFINE PYRAMID TRANSFORMER MODELS"""
"""""" """""" """""" """""" """""" """""" """"""
...
@register_model
def pvt_v2_b0(opt, pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'pvt_v2_b0.pth'))
...
@register_model
def pvt_v2_b1(opt, pretrained=False, **kwargs):
    model = PyramidVisionTransformerV2(
        img_size=opt.imagesize,
        patch_size=4,
        in_chans=3,
    ...
    if pretrained:
        state_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'pvt_v2_b1.pth'))
        del state_dict['head.weight']
        del state_dict['head.bias']
        model.load_state_dict(state_dict, strict=True)
...
@register_model
def pvt_v2_b2(opt, pretrained=False, **kwargs):
    model = PyramidVisionTransformerV2(
        img_size=opt.imagesize,
        patch_size=4,
        in_chans=3,
    ...
    if pretrained:
        state_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'pvt_v2_b2.pth'))
        del state_dict['head.weight']
        del state_dict['head.bias']
...
@register_model
def pvt_v2_b3(opt, pretrained=False, **kwargs):
    model = PyramidVisionTransformerV2(
        img_size=opt.imagesize,
        patch_size=4,
        in_chans=3,
    ...
    if pretrained:
        state_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'pvt_v2_b3.pth'))
        del state_dict['head.weight']
        del state_dict['head.bias']
        model.load_state_dict(state_dict, strict=True)
...
@register_model
def pvt_v2_b4(opt, pretrained=False, **kwargs):
    model = PyramidVisionTransformerV2(
        img_size=opt.imagesize,
        patch_size=4,
        in_chans=3,
...
@register_model
def pvt_v2_b5(opt, pretrained=False, **kwargs):
    model = PyramidVisionTransformerV2(
        img_size=opt.imagesize,
        patch_size=4,
        in_chans=3,
...
@register_model
def pvt_v2_b2_li(opt, pretrained=False, **kwargs):
    model = PyramidVisionTransformerV2(
        img_size=opt.imagesize,
        patch_size=4,
        in_chans=3,
    ...
    if pretrained:
        state_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'pvt_v2_b2_li.pth'))
        del state_dict['head.weight']
        del state_dict['head.bias']
        model.load_state_dict(state_dict, strict=True)
