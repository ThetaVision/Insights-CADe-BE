...
Vision Mamba Implementation adapted from:
...
@register_model
# Original name: vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2
def vim_tiny(opt, pretrained=False, **kwargs):
    model = VisionMamba(
    ...
    if pretrained:
        state_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'vim_t_midclstok_76p1acc.pth'))['model']
...
@register_model
# Original name: vim_tiny_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2
def vim_tiny_stride8(opt, pretrained=False, **kwargs):
    model = VisionMamba(
        img_size=opt.imagesize,
        patch_size=16,
        stride=8,
...
@register_model
# Original name: vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2
def vim_small(opt, pretrained=False, **kwargs):
    model = VisionMamba(
        img_size=opt.imagesize,
        patch_size=16,
        stride=16,
    ...
    model.default_cfg = _cfg()
    if pretrained:
        state_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'vim_s_midclstok_80p5acc.pth'))['model']
        del state_dict['pos_embed']
        del state_dict['head.weight']
        del state_dict['head.bias']
        model.load_state_dict(state_dict, strict=False)
...
@register_model
# Original name: vim_small_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2
def vim_small_stride8(opt, pretrained=False, **kwargs):
    model = VisionMamba(
        img_size=opt.imagesize,
        patch_size=16,
        stride=8,
...
if __name__ == '__main__':
    ...
    model = vim_tiny_stride8(opt=opt, pretrained=True)
    from torchinfo import summary

    summary(model, input_size=(1, 3, 256, 256))

    model.cuda()
    dummy = torch.ones((1, 3, 256, 256)).cuda()
    out = model(dummy)
    print(out.shape)
