...
class ESFPNetStructure(nn.Module):
    def __init__(self, opt, embedding_dim=160):
        ...
        self.linear_fuse34 = ConvModule(
            in_channels=(self.MiT.embed_dims[2] + self.MiT.embed_dims[3]),
            out_channels=self.MiT.embed_dims[2],
            kernel_size=1,
            norm_cfg=dict(type='BN', requires_grad=True),
        )
        self.linear_fuse23 = ConvModule(
            in_channels=(self.MiT.embed_dims[1] + self.MiT.embed_dims[2]),
            out_channels=self.MiT.embed_dims[1],
            kernel_size=1,
            norm_cfg=dict(type='BN', requires_grad=True),
        )
        self.linear_fuse12 = ConvModule(
            in_channels=(self.MiT.embed_dims[0] + self.MiT.embed_dims[1]),
            out_channels=self.MiT.embed_dims[0],
            kernel_size=1,
            norm_cfg=dict(type='BN', requires_grad=True),
        )
        ...
        self.linear_pred = nn.Conv2d(
            (self.MiT.embed_dims[0] + self.MiT.embed_dims[1] + self.MiT.embed_dims[2] + self.MiT.embed_dims[3]),
            1,
            kernel_size=(1, 1),
        )
    ...
    def forward(self, x):
        ...
        out_1 = (
            out_1.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        )  # (Batch_Size, self.MiT.embed_dims[0], 88, 88)
        ...
        out_2 = (
            out_2.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        )  # (Batch_Size, self.MiT.embed_dims[1], 44, 44)
        ...
        out_3 = (
            out_3.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        )  # (Batch_Size, self.MiT.embed_dims[2], 22, 22)
        ...
        out_4 = (
            out_4.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        )  # (Batch_Size, self.MiT.embed_dims[3], 11, 11)
