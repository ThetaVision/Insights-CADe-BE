"""
UniFormer Implementation adapted from:
...
@register_model
def uniformer_small(opt, pretrained=True, **kwargs):
    model = UniFormer(
        depth=[3, 4, 8, 3],
    ...
    if pretrained:
        state_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'uniformer_small_in1k.pth'))['model']
...
@register_model
def uniformer_small_plus(opt, pretrained=True, **kwargs):
    model = UniFormer(
        depth=[3, 5, 9, 3],
        img_size=opt.imagesize,
        in_chans=3,
    ...
    if pretrained:
        state_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'uniformer_small_plus_dim64_in1k.pth'))['model']
        del state_dict['head.weight']
        del state_dict['head.bias']
        model.load_state_dict(state_dict, strict=False)
...
@register_model
def uniformer_base(opt, pretrained=True, **kwargs):
    model = UniFormer(
        depth=[5, 8, 20, 7],
        img_size=opt.imagesize,
        in_chans=3,
    ...
    if pretrained:
        state_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'uniformer_base_in1k.pth'))['model']
        del state_dict['head.weight']
        del state_dict['head.bias']
        model.load_state_dict(state_dict, strict=False)
...
@register_model
def uniformer_base_ls(opt, pretrained=True, **kwargs):
    global layer_scale
    layer_scale = True
    model = UniFormer(
        depth=[5, 8, 20, 7],
    ...
    if pretrained:
        state_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'uniformer_base_ls_in1k.pth'))['model']
        del state_dict['head.weight']
        del state_dict['head.bias']
        model.load_state_dict(state_dict, strict=False)
