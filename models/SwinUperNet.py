...
class SwinUperNet(nn.Module):
    def __init__(self, opt):
        ...
        # Implement and initialize backbone
        if 'Swin-T' in opt.backbone:
            self.swinv2 = SwinTransformerV2(
            ...
            pretrained_path = os.path.join(
                os.getcwd(),
                'pretrained',
                'swinv2_small_patch4_window8_256.pth',
            )
            self._load_pretrained(pretrained_path=pretrained_path)
            feature_channels = [192, 384, 768, 768]

        elif 'Swin-B' in opt.backbone:
            self.swinv2 = SwinTransformerV2(
                img_size=opt.imagesize,
                window_size=8,
                embed_dim=128,
        ...
        self.fc = nn.Linear(feature_channels[-1], opt.num_classes)
    ...
    def forward(self, x):
        ...
        # Extract features
        features = self.swinv2.extra_features(x)
        features_cls = features[-1]

        # UperNet
        features[-1] = self.PPN(features[-1])
        features_seg = self.head(self.FPN(features))

        # Segmentation
        seg = F.interpolate(features_seg, size=input_size, mode='bilinear')

        # Classification branch
        cls = self.avgpool(features_cls)
    ...
    def _load_pretrained(self, pretrained_path):
        # Specify device and load pretrained dict
        # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        # pretrained_dict = torch.load(pretrained_path, map_location=device)
        pretrained_dict = torch.load(pretrained_path)

        # If the model key is in pretrained dict
        pretrained_dict = pretrained_dict['model']
        state_dict = self.swinv2.state_dict()
        model_dict = {}
        for k, v in pretrained_dict.items():
            if k in state_dict and 'head.weight' not in k and 'head.bias' not in k:
                model_dict[k] = v
        self.swinv2.load_state_dict(model_dict, strict=False)


"""""" """""" """""" """""" """""" """"""
"""" DEFINE HELPER FUNCTIONS (SWIN)"""
"""""" """""" """""" """""" """""" """"""
# --------------------------------------------------------
# Swin Transformer V2
# Copyright (c) 2022 Microsoft
...
# Written by Ze Liu
# --------------------------------------------------------
# Original swin v2 from https://github.com/microsoft/Swin-Transformer/blob/main/models/swin_transformer_v2.py
# Adapted from https://www.kaggle.com/code/abebe9849/swin-v2-unet-upernet
# Weights downloaded from https://github.com/microsoft/Swin-Transformer
...
def swin_v2_tiny(opt, pretrained=False):
    ...
    if pretrained:
        pretrained_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'swinv2_tiny_patch4_window8_256.pth'))[
            'model'
        ]
...
def swin_v2_small(opt, pretrained=False):
    model = SwinTransformerV2(
        img_size=opt.imagesize,
        patch_size=4,
        in_chans=3,
    ...
    if pretrained:
        pretrained_dict = torch.load(os.path.join(os.getcwd(), 'pretrained', 'swinv2_small_patch4_window8_256.pth'))[
            'model'
        ]
...
"""""" """""" """""" """""" """""" """"""
"""" DEFINE HELPER FUNCTIONS (UperNet)"""
"""""" """""" """""" """""" """""" """"""
