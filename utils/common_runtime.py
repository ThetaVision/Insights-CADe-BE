...
def allocate_buffers(engine: trt.ICudaEngine, 
    ...
    Args:
        ...
        context (Optional[trt.IExecutionContext]): Execution context with resolved shapes.
        profile_idx (Optional[int]): Profile index to use when getting binding shapes.

    Returns:
        inputs (list): Host memory buffers for inputs.
        outputs (list): Host memory buffers for outputs.
        bindings (list): Device memory pointers for all bindings.
        stream: CUDA stream handle.
    """
    inputs = []
    outputs = []
    bindings = []
    # Create a CUDA stream (assuming cuda_call and cudart.cudaStreamCreate() are defined)
    stream = cuda_call(cudart.cudaStreamCreate())
    
    tensor_names = [engine.get_tensor_name(i) for i in range(engine.num_io_tensors)]
    
    for binding in tensor_names:
        # Use the execution context if provided to get the static shape.
        if context is not None:
            shape = context.get_tensor_shape(binding)
        elif profile_idx is None:
            shape = engine.get_tensor_shape(binding)
        else:
            # get_tensor_profile_shape returns (min_shape, optimal_shape, max_shape)
            shape = engine.get_tensor_profile_shape(binding, profile_idx)[-1]
        
        # Check if the shape is valid (all dimensions non-negative)
        shape_valid = np.all([s >= 0 for s in shape])
        if not shape_valid and context is None and profile_idx is None:
        ...
        # Append the device buffer pointer to the bindings list.
        bindings.append(int(bindingMemory.device))
        
        # Append to the appropriate list based on I/O mode.
